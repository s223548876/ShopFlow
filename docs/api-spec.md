...
- Request 中的 `userId`、`role`、商品價格及訂單總價一律忽略或拒絕，不能作為授權或計價依據
...
由目前使用者的整個購物車建立訂單。沒有 request body；不接受商品清單、價格、總價或 userId。
...
Query：`page`、`size`、`sort`，預設 `createdAt,desc`。只回傳目前 JWT 使用者的訂單。
...
Query：`q`、`categoryId`、`active`、`page`、`size`、`sort`。未指定 `active` 時同時回傳 active 與 inactive。
...
名稱長度 1–200、description 不超過 5000、price 大於 0、stock 不得為負，category 必須存在。

成功：`201 Created`，回傳 AdminProductResponse，`active` 預設 true。

錯誤：`400 VALIDATION_ERROR`、`404 CATEGORY_NOT_FOUND`。
...
將 `active` 設為 false，不物理刪除。成功及重複停用皆回傳 `204 No Content`；不存在回傳 `404 PRODUCT_NOT_FOUND`。
...
Query：`status`、`page`、`size`、`sort`，預設 `createdAt,desc`。成功回傳包含所有使用者訂單的 PageResponse<OrderSummaryResponse>。
...
`PENDING_PAYMENT → PAID` 只能由訂單本人呼叫模擬付款 endpoint 完成。相同狀態的重複 request 回傳目前 OrderResponse，不重複執行副作用。
