...
Vite `/api`、`/actuator` proxy 只用於本機 development server，避免前端開發時硬編碼 backend URL。

```mermaid
flowchart LR
    Browser["瀏覽器"] --> Vite["Vue 3 + Vite development server"]
    Vite -->|"/api、/actuator proxy"| Backend["Spring Boot backend"]
...
Docker Compose 啟動 `frontend`、`backend`、`postgres` 三個 service。Frontend image 先建置 Vue 靜態檔，再由 Nginx 提供；Nginx 將 `/api`、`/actuator` 反向代理至 Docker network 內的 backend service，其餘路徑回傳 SPA 靜態檔並支援 Vue Router history fallback。

```mermaid
flowchart LR
    Browser["瀏覽器"] -->|"HTTP"| Nginx["Nginx + Vue 靜態檔"]
    Nginx -->|"/api、/actuator reverse proxy"| Backend["Spring Boot backend"]
...
Vite proxy 與 Nginx reverse proxy 是互斥的執行路徑：前者只服務直接開發，後者服務完整 Compose。正式 runtime 不啟動 Vite development server。Swagger UI 與 OpenAPI JSON 直接由 backend port 提供，不經 Nginx 或 Vite proxy。

## 後端模組

| 模組 | 責任 |
| --- | --- |
| `auth` | 註冊、登入、BCrypt 與 User／Role |
| `catalog` | Category、公開商品查詢、搜尋、篩選與分頁 |
| `cart` | 使用者自己的 Cart 與 CartItem |
| `order` | 下單交易、OrderItem 快照、模擬付款與自己的訂單查詢 |
| `admin` | 商品管理、絕對庫存設定、全部訂單查詢及狀態管理 |
| `common` | 共用 PageResponse、全域錯誤處理、OpenAPI、Spring Security、JWT 建立與 principal 解析 |
...
| --- | --- |
| `/api/auth/**` | 公開 |
| `GET /api/categories`、`GET /api/products`、`GET /api/products/**` | 公開 |
| `/actuator/health` | 公開；其他 Actuator endpoint 不暴露 |
| `/swagger-ui.html`、`/swagger-ui/**`、`/v3/api-docs/**` | 公開文件 |
| `/api/cart`、`/api/cart/**` | 僅 CUSTOMER |
| `/api/orders`、`/api/orders/**` | 僅 CUSTOMER |
| `/api/admin/**` | 僅 ADMIN |
| 其他 `/api/**` | 需要有效 JWT |
| 其他 request | `anyRequest().denyAll()` |

公開 Swagger 不會放寬 API runtime 權限。OpenAPI tags 為 Authentication、Catalog、Cart、Customer Orders、Admin Products、Admin Orders、Health。
...
Repository 已提供只含 placeholder 的 `.env.example`。Docker Compose 從 `.env` 讀取 PostgreSQL、JWT secret 與 port 設定；直接執行 Spring Boot 時，`DB_URL`、`DB_USERNAME`、`DB_PASSWORD`、`JWT_SECRET` 必須由 shell 環境提供。實際 `.env` 不得提交。

目前瀏覽器透過 Vite 或 Nginx 同源 proxy 存取 backend，專案未啟用 CORS 設定。
