...
目前已完成 repository 品質基線與本機基礎設施：Java 21 / Spring Boot 3.5 backend、Vue 3 / TypeScript / Vite frontend、PostgreSQL、Flyway、Docker Compose、Nginx 與 Actuator health check。尚未建立任何業務資料表、Entity、JWT 或商業 API。
...
backend 啟動時會自動執行 `classpath:db/migration` 的 Flyway migration。目前 migration 只建立 Flyway schema history，不建立業務資料表。
...
目前沒有商業 Controller，因此 `/api` 回傳 404 是預期行為；只有 `/actuator/health` 已提供。
